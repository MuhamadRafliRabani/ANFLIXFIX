...
const Navbar = () => {
  ...
  const UserComp = () => {
    return (
      <div className="flex gap-4 items-center">
        <div
          className={`relative main-transition ${
        ...
        >
          ...
          />
          <div
            className={`w-[250px] h-fit font-medium text-start absolute top-full main-transition origin-top right-10 ps-4 bg-black bg-opacity-70 rounded-md text-base shadow-md ${
              isOpenDas ? "" : "scale-0"
            }`}
          >
            <Link
  ...
  const SearchBtn = () => {
    return (
      <button
        className={`w-fit ${isOpen && "absolute top-6 right-0 md:static"}`}
      >
        {!isOpen ? (
          <MagnifyingGlass
            className="size-10 md:size-10 md:py-1 px-2 rounded-full hover:bg-primary"
  ...
  return (
    <header className="bg-transparent fixed top-0 left-0 right-0 z-50">
      <nav className="md:px-4 py-2 px-2 flex justify-between items-center text-white relative">
        ...
        <div
          ref={searchRef}
          className={`absolute left-0 w-4/5 main-transition z-40 md:w-full ${
